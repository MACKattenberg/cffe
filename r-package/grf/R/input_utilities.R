...
create_train_matrices <- function(X,
  ...
  if (!is.null(individual_fe)) {
    out[["individual_fe.index"]] <- offset + 1
    offset <- offset + 1
  }
  if (!is.null(time_fe)) {
    out[["time_fe.index"]] <- offset + 1
    offset <- offset + 1
  }
  if (!is.null(event_time_fe)) {
	out[["event_time_fe.index"]] <- offset + 1
	offset <- offset + 1
  }
  ...
  X <- as.matrix(X)
  out[["train.matrix"]] <- as.matrix(cbind(X, outcome, treatment, instrument, survival.numerator, survival.denominator, censor, individual_fe, time_fe, event_time_fe, sample.weights))
