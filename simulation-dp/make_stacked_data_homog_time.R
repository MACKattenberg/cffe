make_stacked_data_homog_time <- function(
    n_individuals = 5000,
    n_periods = 10,
  ...
  for (k in kappas) {
    for (l in lambdas) {
      data[, new_y := yvar + k * (fe + treat1 + treat2) - l * t]
  ...
  # check
  stopifnot(data[, y_kaplam00 - 
                   (treat_effect * posttreat + X2^2  + X1^2 + epsilon)] == 0)
